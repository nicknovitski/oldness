...
module Oldness
  ...
  def Oldness::date_of_first_work(medium)
    case medium
  ...
  def Oldness::rating(datestring, medium)
    current_date=Date.today
    work_date = Oldness::date_from(datestring)
    #raise ArgumentError if work_date > current_date or work_date < start_date
    Oldness::ranges(medium).find {|key, range| range.cover?(work_date)}[0]
  end

  def Oldness::ranges(medium)
    start_date = Oldness::date_of_first_work(medium)
    current_date = Date.today
